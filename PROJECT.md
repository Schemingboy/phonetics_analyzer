...
- 在发生失爆的词尾爆破字母 `p / b / t / d / k / g` 右上角加 `˺` 上标。
- `˺` 表示“闭住/卡住但不爆开”，不要和 `elision` 的删除线混用。
...
- 语流现象用接近正文的英文短标记：`A‿B link`、`far‿away r-link`、浅灰 `to weak`、`d→j assimilate`、删除线 `x sound drop`、`d˺ hold`、`idea‿r‿of insert-r`。
...
- elision 删除线和图例使用 `--c-elision`，弱化仍保留灰色，二者视觉必须区分。
- 示例句按钮点击后会直接填入句子并触发分析。
- `schwa cue` 必须由实际 IPA 内容确认，不因内容词词表命中而无条件显示。
- 分析加载期间旧结果区进入 `.stale` 状态；成功或失败后都必须恢复正常显示。
- `lost_plosive` 使用 `˺` 上标，不再依赖 CSS 伪元素斜杠。
- 真实 API 回归中，`hot dog / good morning / I wanted to go, but I changed my mind. / I need all of it out of the oven in about an hour.` 均能返回并渲染；多词 `lost_plosive` segment 只标边界前词，避免 `hot dog` 误显示成 `hot˺ dog˺`。
- TTS 引擎选择器常驻分析结果区；详细 Key/voice 配置仍保留在 API 设置卡。
- browser TTS 播放时当前 sense group 用浅蓝背景高亮；外部 TTS MVP 只高亮第一个 sense group，播放结束后清除。
...
- 优先做 B-6：用 `ResizeObserver` 替换双 RAF + `setTimeout`，减少语调曲线对布局时机的依赖。
