...
v14.19 已新增 `schwa cue` 和 `focus cue` 的前端派生 CUES 层，并完成 Batch 1 零风险 bug 修复。当前重点是做真实 API 回归，确认新增 cue 足够轻量，且不干扰既有 FLOW 标注和语调曲线。
...
- 真实 API 回归中重点检查 `schwa cue` 是否只出现在高价值弱读点，不形成满屏噪音。
